...
### Securing the prayer-push cron

`send-prayer-pushes` is meant to be triggered only by your scheduled job (pg_cron / Supabase
Cron), never by a public caller. It requires:

- An Edge Function secret named `CRON_SECRET` (`supabase secrets set CRON_SECRET=<your-value>`).
  Never commit its value — this repo only documents the variable name.
- The cron trigger configured to send it back as a request header on every call:
  `x-cron-secret: <same value>`.

Wherever the cron job itself is defined (Supabase Dashboard → Database → Cron Jobs, or a
`cron.schedule(...)` / `pg_net.http_post` call), add that header to its request — otherwise the
function will reject every call, including the legitimate scheduled one, with `401`.
