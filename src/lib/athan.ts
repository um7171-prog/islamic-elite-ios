...
function extraBody(kind: ExtraAlertKind, lang: "en" | "ar", mins: number, prayerAr: string, prayerEn: string) {
  if (lang === "ar") {
    ...
    if (kind === "midnight") return { title: "منتصف الليل الشرعي", body: "وقت قيام الليل والدعاء" };
    if (kind === "witr") return { title: "صلاة الوتر", body: "لا تنسَ الوتر قبل الفجر" };
    if (kind === "qiyam") return { title: "قيام الليل", body: "قم فصلِّ ما تيسر لك" };
  ...
  if (kind === "midnight") return { title: "Islamic midnight", body: "Time for Qiyam al-Layl" };
  if (kind === "witr") return { title: "Witr prayer", body: "Don't forget Witr before Fajr" };
  if (kind === "qiyam") return { title: "Qiyam al-Layl", body: "Stand for the night prayer" };
...
function playVoice() {
  try {
    const audio = new Audio(PRE_REMINDER_VOICE_URL);
    audio.volume = 1;
    void audio.play().catch(() => {});
  } catch {}
}
...
export function scheduleAthan({ entries, settings, lang, sunnah = null, now = new Date() }: ScheduleOptions): ScheduleHandle {
  ...
  if (sunnah) {
    const na = settings.nightAlerts;
    const legacy = settings.nightAlertsEnabled;
    const fajr = entries.find((e) => e.key === "fajr");
    const plan: [ExtraAlertKind, Date | undefined, boolean, number][] = [
      ["midnight", sunnah.middleOfTheNight, legacy || !!na?.midnight?.enabled, na?.midnight?.offsetMinutes ?? 0],
      ["lastThird", sunnah.lastThirdOfTheNight, legacy || !!na?.lastThird?.enabled, na?.lastThird?.offsetMinutes ?? 0],
      ["witr", fajr ? new Date(fajr.time.getTime() - 45 * 60_000) : undefined, !!na?.witr?.enabled, na?.witr?.offsetMinutes ?? 0],
      ["qiyam", sunnah.lastThirdOfTheNight, !!na?.qiyam?.enabled, na?.qiyam?.offsetMinutes ?? 0],
    ];
    for (const [kind, when, on, offset] of plan) {
      if (!on || !when) continue;
      scheduleAt(when.getTime() - offset * 60_000, () => fireExtra(kind));
    }
  }
