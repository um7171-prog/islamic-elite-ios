...
export function QuranDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (v: boolean) => void }) {
  ...
  useEffect(() => {
    ...
    fetch("https://api.alquran.cloud/v1/surah").then(r => r.json()).then(j => setSurahs(j.data || [])).catch(() => {});
  }, [open, surahs.length]);
        ...
        {!active ? (
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-[55vh] overflow-y-auto">
              ...
              {!surahs.length && <div className="col-span-full text-center text-xs text-foreground/50 py-8"><Loader2 className="inline h-4 w-4 animate-spin" /> {t("Loading…", "جاري التحميل…")}</div>}
            </div>
          </>
        ) : (
          <div
