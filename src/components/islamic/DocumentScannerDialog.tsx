...
export function DocumentScannerDialog({ open, onOpenChange }: Props) {
  ...
  const start = useCallback(async () => {
    ...
    try {
      ...
      if (name === "NotAllowedError" || name === "SecurityError") {
        ...
        setError(t("Camera permission denied. Enable it in browser settings.", "تم رفض إذن الكاميرا. فعّله من إعدادات المتصفح."));
  ...
  const savePdf = async () => {
    ...
    a.href = url; a.download = `scan-${Date.now()}.pdf`; a.click();
    setTimeout(() => URL.revokeObjectURL(url), 1000);
    toast.success(t("PDF saved", "تم حفظ PDF"));
  };
  ...
  const sharePdf = async () => {
    ...
    if (nav.canShare?.({ files: [file] })) {
      try { await navigator.share({ files: [file], title: t("Scanned document", "مستند ممسوح") }); }
      catch { /* user canceled */ }
    } else {
      await savePdf();
                ...
                <div key={p.id} className="rounded-xl border border-border bg-card p-2">
                  <div className="flex items-center justify-between mb-2 px-1">
                    ...
                    <div className="flex gap-1">
                      <button onClick={() => rotatePage(p.id)} className="h-8 w-8 grid place-items-center rounded-md hover:bg-muted" aria-label="rotate">
                        <RotateCw className="h-4 w-4" />
                      </button>
                      <button onClick={() => removePage(p.id)} className="h-8 w-8 grid place-items-center rounded-md hover:bg-destructive/10 text-destructive" aria-label="delete">
