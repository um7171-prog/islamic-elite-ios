...
export function HeroPrayerCard() {
  ...
  return (
    <div
      className="relative overflow-hidden rounded-card-lg px-5 py-6 md:px-8 md:py-7 border border-white/10 shadow-[var(--shadow-elevated)]"
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center text-center">
        <span className="inline-flex items-center gap-1.5 rounded-full bg-white/10 px-3 py-1 text-label text-white/80">
          <MoonStar className="h-3.5 w-3.5 text-elite-gold" />
          {t("Next Prayer", "الصلاة القادمة")}
        </span>
        <div className="mt-2 font-arabic text-h1 md:text-4xl text-white">
      ...
      <div className={`relative z-10 mt-3 grid ${showElapsed ? "grid-cols-2" : "grid-cols-1"} gap-3 items-start text-center`}>
        {/* Next prayer countdown — the single most prominent number on Home */}
        <div className="flex flex-col items-center">
          <div className="font-time text-display md:text-5xl text-elite-gold tracking-tight tabular-nums">
            {formatHMS(msUntilNext)}
          </div>
          <div className="mt-1 font-arabic text-caption md:text-body-sm text-white/60">
            {t("remaining", "متبقٍ")}
          </div>
        </div>
        ...
        {showElapsed && (
          <div className="flex flex-col items-center border-s border-white/15">
            <div className="font-arabic text-body-sm md:text-h3 font-semibold text-white/90">
              {t(`Since ${current.nameEn}`, `${current.nameAr} منذ`)}
            </div>
            <div className="mt-1 font-time text-h1 md:text-4xl font-bold text-white tracking-tight tabular-nums">
