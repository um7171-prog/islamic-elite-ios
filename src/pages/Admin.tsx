...
import { Loader2, LogOut, Users, Download, Globe, Smartphone, Megaphone, Trash2, Shield, Mail, Lock, Eye, EyeOff, RefreshCw, BarChart2, CheckCircle, AlertCircle } from "lucide-react";
...
function Dashboard({ session, onLogout }: { session: Session; onLogout: () => void }) {
  ...
  if (!isAdmin) {
    return (
      <div dir="rtl" className="min-h-screen grid place-items-center px-4">
  ...
  return (
    <div dir="rtl" className="min-h-screen px-4 py-6 max-w-6xl mx-auto" style={{ paddingTop: "calc(env(safe-area-inset-top, 0px) + 1.5rem)" }}>
      ...
      <Tabs defaultValue="overview">
        ...
        <TabsContent value="overview" className="space-y-4 pt-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            <StatCard icon={Download} label="تحميلات تيك توك" value={toolStats.tiktok} />
            <StatCard icon={Download} label="تحميلات يوتيوب" value={toolStats.youtube} />
            <StatCard icon={BarChart2} label="أدوات أخرى" value={toolStats.other} />
          </div>
...
export default function Admin() {
  ...
  if (!ready) return <div className="min-h-screen grid place-items-center"><Loader2 className="animate-spin h-6 w-6" /></div>;
